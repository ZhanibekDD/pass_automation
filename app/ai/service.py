...
from app.ai.schemas import DocumentSnapshot, EmployeeSnapshot, RuleIssue
...
class DocumentAIService:
    ...
    def check_completeness(
        self, employee: EmployeeSnapshot, *, persist: bool = True
    ) -> dict[str, Any]:
        existing_codes = {document.document_code for document in employee.documents}
        missing = [
            code
            for code in self.settings.required_document_codes
        ...
        return {
            ...
            "missing_documents": [
                {"document_code": code, "name": DOC_CATALOG.get(code, "Неизвестно")}
                for code in missing
            ],
    ...
    def analyze_document(
        self, employee: EmployeeSnapshot, document: DocumentSnapshot
    ) -> dict[str, Any]:
